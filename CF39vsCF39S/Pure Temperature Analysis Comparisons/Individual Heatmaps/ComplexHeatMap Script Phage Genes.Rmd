...
LoadedHeatShockData <- read_delim("E:/Dropbox/Dropbox/Harrison Lab - Trevor Randall/RNASeq Analysis/RNASeqAnlyPkrat_2020_03/CF39vsCF39S/Pure Temperature Analysis Comparisons/Individual Heatmaps/Cut Data Sets/Phage Genes Full Dataset.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
...
FinalColumnNames <- append(colnames(DataSet)[1:6], c("NucleotidePAO1Locus", "NucleotidePAO1ID", "ProteinPAO1Locus", "ProteinPAO1ID"))
...
```{r}
png("PhageGenes_Clustered_1.png", width = 20, height = 10, res = 300, units = "cm")
HeatmapTest
dev.off()
```
...
#Session Info
```{r}
sink(file = "./PhageGenesSessionInfo.txt")
sessionInfo()
sink(file = NULL)
```
...
```{r}
sink(file = "./PhageGenesFlagellaReferenceInfo.txt")
citation("readr")
