...
#Set working directory
#PC Path
setwd("E:/Dropbox/Dropbox/Harrison Lab - Trevor Randall/RNASeq Analysis/RNASeqAnlyPkrat_2020_03/CF39vsCF39S/29 vs 29/DE_Reg_Genes_Analysis/")
#Laptop Path
#Set Library Directory
#.libPaths(c(""))
#setwd("")
...
```{r}
TDESeq_29vs29 <- read_delim("../My DESEQ Script/DESeqNew_results.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
colnames(TDESeq_29vs29) <- ColNameLablesDESeqTrevor
...
```{r}
TDESeq_29vs29 <-merge(TDESeq_29vs29, LocusGeneAssociation, by ="locus")
...
```{r}
TDESEQ29vs29 <- TDESeq_29vs29[((TDESeq_29vs29$padj <PAdjValue)& (!is.na(TDESeq_29vs29$padj))), ]

write.table(x = TDESEQ29vs29, file="./TDESEQ29vs29.txt", sep="\t", row.names=FALSE, col.names=colnames(TDESEQ29vs29), quote=F)
```
