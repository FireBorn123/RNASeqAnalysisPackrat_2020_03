...
#Set working directory
#PC Path
setwd("E:/Dropbox/Dropbox/Harrison Lab - Trevor Randall/RNASeq Analysis/RNASeqAnlyPkrat_2020_03/CF39vsCF39S/33 vs 33/DE_Reg_Genes_Analysis/")
#Laptop Path
#Set Library Directory
#.libPaths(c(""))
#setwd("")
...
```{r}
TDESeq_33vs33 <- read_delim("../My DESEQ Script/DESeqNew_results.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
colnames(TDESeq_33vs33) <- ColNameLablesDESeqTrevor
...
```{r}
TDESeq_33vs33 <-merge(TDESeq_33vs33, LocusGeneAssociation, by ="locus")
...
```{r}
TDESEQ33vs33 <- TDESeq_33vs33[((TDESeq_33vs33$padj <PAdjValue)& (!is.na(TDESeq_33vs33$padj))), ]

write.table(x = TDESEQ33vs33, file="./TDESEQ33vs33.txt", sep="\t", row.names=FALSE, col.names=colnames(TDESEQ33vs33), quote=F)
```
